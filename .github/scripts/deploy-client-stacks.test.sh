...
for scenario in healthy stopped restarting unhealthy old-image bad-compose old-config legacy-image pull-error missing-image mutable-image wrong-repo malformed-sha; do
  rm -f "$work_dir/mutated" "$work_dir/api-called"
  if (
    ...
    export PORTLESS_CLIENT_DEPLOY_IMAGE=ghcr.io/crbl-technologies/portless-client:sha-0123456789abcdef0123456789abcdef01234567
    case "$scenario" in
      missing-image) unset PORTLESS_CLIENT_DEPLOY_IMAGE;;
      mutable-image) export PORTLESS_CLIENT_DEPLOY_IMAGE=ghcr.io/crbl-technologies/portless-client:prod;;
      wrong-repo) export PORTLESS_CLIENT_DEPLOY_IMAGE=ghcr.io/other/client:sha-0123456789abcdef0123456789abcdef01234567;;
      malformed-sha) export PORTLESS_CLIENT_DEPLOY_IMAGE=ghcr.io/crbl-technologies/portless-client:sha-invalid;;
    esac
    ...
    curl() {
      ...
      case "$url" in
        ...
        */images/create\?*)
          [ "$url" = "https://portainer.invalid/api/endpoints/3/docker/images/create?fromImage=ghcr.io%2Fcrbl-technologies%2Fportless-client&tag=sha-0123456789abcdef0123456789abcdef01234567" ] || return 1
          local output="" previous="" arg
          for arg in "$@"; do
            [ "$previous" != -o ] || output="$arg"
            previous="$arg"
          done
          if [ "$scenario" = pull-error ]; then
        ...
        */containers/*/json)
          jq -n --arg scenario "$scenario" --arg hash "$expected_config_hash" --arg image "$PORTLESS_CLIENT_DEPLOY_IMAGE" '{
            Config:{Image:$image, Labels:{"com.docker.compose.config-hash":(if $scenario == "old-config" then "old" else $hash end)}},
        ...
        */stacks/1\?*)
          jq -e --arg image "$PORTLESS_CLIENT_DEPLOY_IMAGE" '.env | any(.name == "PORTLESS_CLIENT_IMAGE" and .value == $image)' "$payload_file" >/dev/null || return 1
    ...
    sleep() { :; }
    export -f curl sleep
    export scenario work_dir
    bash .github/scripts/deploy-client-stacks.sh
  ...
  case "$scenario" in
    legacy-image|pull-error|missing-image|mutable-image|wrong-repo|malformed-sha)
      [ ! -e "$work_dir/mutated" ] || { echo "Preflight failure mutated the stack: $scenario" >&2; exit 1; };;
  esac
  case "$scenario" in
    missing-image|mutable-image|wrong-repo|malformed-sha)
      [ ! -e "$work_dir/api-called" ] || { echo "Invalid image reached the API: $scenario" >&2; exit 1; };;
  esac
done
